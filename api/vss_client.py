...
def check_err_response(response: requests.Response, exit_on_error=False):
    if response.status_code >= 400:
        ...
        # Safely extract common error fields if present
        code = err_json.get('code') or err_json.get('status') or None
        if not code and isinstance(err_json.get('error'), dict):
            code = err_json['error'].get('code')

        if 'message' in err_json:
            message = err_json['message']
        elif 'detail' in err_json:
            message = err_json['detail']
        ...
        print(f"Request failed, status: {response.status_code}, code: {code}, message: {message}")
        if exit_on_error:
            sys.exit(-1)
...
def call_vss_chat(
    ...
    """Call VSS chat/completions API and return parsed response.

    Args:
        asset_ids: List of asset IDs or single ID string
        model: Model name to use
        prompt: User prompt/question
        backend: VSS backend URL (default: http://localhost:8000)
        temperature, seed, top_p, top_k, max_tokens: Model parameters
        chunk_duration: Chunk duration in seconds
        enable_reasoning: Enable reasoning in response
        stream: Enable streaming (default: False)

    Returns:
        dict with 'content' and 'usage' keys if successful, None on error
    """
    global BASE_URL
    ...
    try:
        ...
        req_json = {"id": asset_ids, "model": model}
        ...
        req_json["messages"] = [{"content": str(prompt), "role": "user"}]
        
        response = requests.post(get_api_url("/chat/completions"), json=req_json, stream=stream)
        
        if response.status_code >= 400:
            print(f"VSS Chat API error: {response.status_code}")
            try:
        ...
        return {"content": content, "usage": usage, "full_response": result}
